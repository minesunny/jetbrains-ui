...
export type DbPrimitiveProps = Omit<ComponentProps<'svg'>, 'size'> & {
  size?: 12 | 14 | 16 | 20 | 24;
  mode?: 'light' | 'dark';
};

const DbPrimitiveLight: FC<
...
> = ({
  ...
  >
    ...
    />
    <path
      d="M6.91 6.092H5.858v-4.74h-.84l-1.935.62.256.914 1.303-.381v3.587H3.41v.9h3.5zM11.862 13.74h1.052v.9h-3.5v-.9h1.232v-3.586l-1.303.38-.256-.913 1.935-.62h.84z"
      fill="#6C707E"
    />
  </svg>
);

const DbPrimitiveDark: FC<
  Omit<ComponentProps<'svg'>, 'size'> & { size: number }
> = ({
  size,
  className,
...
export const DbPrimitive: FC<DbPrimitiveProps> = ({
  size = 16,
  mode = 'light',
  className,
  'aria-label': ariaLabel,
  ...
  const SvgComponent = mode === 'light' ? DbPrimitiveLight : DbPrimitiveDark;
