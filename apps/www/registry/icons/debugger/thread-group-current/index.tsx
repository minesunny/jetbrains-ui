import type { FC } from 'react';
import { cn } from '@/lib/utils';
import { type SvgProps, sizeMap } from '../types';

export type ThreadGroupCurrentProps = SvgProps;

const ThreadGroupCurrentLight: FC<SvgProps> = ({
  size,
  className,
  role,
  'aria-label': ariaLabel,
  'aria-hidden': ariaHidden,
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    role={role}
    aria-label={ariaLabel}
    aria-hidden={ariaHidden}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M15.827 11.176a.6.6 0 0 1 0 .848l-3.797 3.797a.6.6 0 0 1-.848 0l-2.006-2.006a.6.6 0 1 1 .848-.848l1.582 1.581 3.373-3.372a.6.6 0 0 1 .848 0"
      fill="#E66D17"
    />
    <path
      d="M12.596 12.144 11.5 10.5v-5l2-3h-11l2 3v5l-2 3h5.504a1.6 1.6 0 0 1 2.728-1.24l.874.874z"
      fill="#EBECF0"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12.957 11.783 12 10.35V5.65l1.916-2.874A.5.5 0 0 0 13.5 2h-11a.5.5 0 0 0-.416.777L4 5.651v4.698l-1.916 2.874A.5.5 0 0 0 2.5 14h5.62a1.6 1.6 0 0 1-.072-1H3.434l1.334-2h6.464l1.003 1.505zM11 6v4H5V6zm.232-1H4.768L3.434 3h9.132z"
      fill="#6C707E"
    />
  </svg>
);

const ThreadGroupCurrentDark: FC<SvgProps> = ({
  size,
  className,
  role,
  'aria-label': ariaLabel,
  'aria-hidden': ariaHidden,
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    role={role}
    aria-label={ariaLabel}
    aria-hidden={ariaHidden}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M15.827 11.176a.6.6 0 0 1 0 .848l-3.797 3.797a.6.6 0 0 1-.848 0l-2.006-2.006a.6.6 0 1 1 .848-.848l1.582 1.581 3.373-3.372a.6.6 0 0 1 .848 0"
      fill="#C77D55"
    />
    <path
      d="M12.596 12.144 11.5 10.5v-5l2-3h-11l2 3v5l-2 3h5.504a1.6 1.6 0 0 1 2.728-1.24l.874.874z"
      fill="#43454A"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M12.957 11.783 12 10.35V5.65l1.916-2.874A.5.5 0 0 0 13.5 2h-11a.5.5 0 0 0-.416.777L4 5.651v4.698l-1.916 2.874A.5.5 0 0 0 2.5 14h5.62a1.6 1.6 0 0 1-.072-1H3.434l1.334-2h6.464l1.003 1.505zM11 6v4H5V6zm.232-1H4.768L3.434 3h9.132z"
      fill="#CED0D6"
    />
  </svg>
);

export const ThreadGroupCurrent: FC<ThreadGroupCurrentProps> = ({
  size = 'md',
  mode = 'light',
  className,
  'aria-label': ariaLabel,
  ...props
}) => {
  const SvgComponent =
    mode === 'light' ? ThreadGroupCurrentLight : ThreadGroupCurrentDark;

  return (
    <SvgComponent
      size={sizeMap[size]}
      className={cn('inline-block flex-shrink-0', className)}
      role={ariaLabel ? 'img' : 'presentation'}
      aria-label={ariaLabel}
      aria-hidden={!ariaLabel}
      {...props}
    />
  );
};
