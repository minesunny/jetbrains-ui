/**
 * Copyright 2000-2024 JetBrains s.r.o. and contributors.
 * Use of this source code is governed by the Apache 2.0 license.
 */
import type { ComponentProps, FC } from 'react';
import { cn } from '@/lib/utils';

export type DatabricksToolWindowProps = Omit<ComponentProps<'svg'>, 'size'> & {
  size?: 12 | 14 | 16 | 20 | 24;
  mode?: 'light' | 'dark';
};

const DatabricksToolWindowLight: FC<
  Omit<ComponentProps<'svg'>, 'size'> & { size: number }
> = ({
  size,
  className,
  role,
  'aria-label': ariaLabel,
  'aria-hidden': ariaHidden,
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    role={role}
    aria-label={ariaLabel}
    aria-hidden={ariaHidden}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M7.79.046a.5.5 0 0 1 .42 0l3.5 1.615a.5.5 0 1 1-.42.908L8 1.051 2.693 3.5 8 5.95l6.29-2.904A.5.5 0 0 1 15 3.5v3a.5.5 0 0 1-.29.454l-6.5 3a.5.5 0 0 1-.42 0L2 7.281v1.9l6 2.768 6.29-2.903A.5.5 0 0 1 15 9.5v3a.5.5 0 0 1-.29.454l-6.5 3a.5.5 0 0 1-.42 0l-6.5-3a.5.5 0 0 1 .42-.908L8 14.949l6-2.769v-1.899l-5.79 2.673a.5.5 0 0 1-.42 0l-6.5-3A.5.5 0 0 1 1 9.5v-3a.5.5 0 0 1 .71-.454L8 8.949l6-2.769V4.281L8.21 6.954a.5.5 0 0 1-.42 0l-6.5-3a.5.5 0 0 1 0-.908z"
      fill="#6C707E"
    />
  </svg>
);

const DatabricksToolWindowDark: FC<
  Omit<ComponentProps<'svg'>, 'size'> & { size: number }
> = ({
  size,
  className,
  role,
  'aria-label': ariaLabel,
  'aria-hidden': ariaHidden,
}) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    role={role}
    aria-label={ariaLabel}
    aria-hidden={ariaHidden}
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M7.79.046a.5.5 0 0 1 .42 0l3.5 1.615a.5.5 0 1 1-.42.908L8 1.051 2.693 3.5 8 5.95l6.29-2.904A.5.5 0 0 1 15 3.5v3a.5.5 0 0 1-.29.454l-6.5 3a.5.5 0 0 1-.42 0L2 7.281v1.9l6 2.768 6.29-2.903A.5.5 0 0 1 15 9.5v3a.5.5 0 0 1-.29.454l-6.5 3a.5.5 0 0 1-.42 0l-6.5-3a.5.5 0 0 1 .42-.908L8 14.949l6-2.769v-1.899l-5.79 2.673a.5.5 0 0 1-.42 0l-6.5-3A.5.5 0 0 1 1 9.5v-3a.5.5 0 0 1 .71-.454L8 8.949l6-2.769V4.281L8.21 6.954a.5.5 0 0 1-.42 0l-6.5-3a.5.5 0 0 1 0-.908z"
      fill="#CED0D6"
    />
  </svg>
);

export const DatabricksToolWindow: FC<DatabricksToolWindowProps> = ({
  size = 16,
  mode = 'light',
  className,
  'aria-label': ariaLabel,
  ...props
}) => {
  const SvgComponent =
    mode === 'light' ? DatabricksToolWindowLight : DatabricksToolWindowDark;

  return (
    <SvgComponent
      size={size}
      className={cn('inline-block shrink-0', className)}
      role={ariaLabel ? 'img' : 'presentation'}
      aria-label={ariaLabel}
      aria-hidden={!ariaLabel}
      {...props}
    />
  );
};

export default DatabricksToolWindow;
