...
export type TestResourcesRootProps = Omit<ComponentProps<'svg'>, 'size'> & {
  size?: 12 | 14 | 16 | 20 | 24;
  mode?: 'light' | 'dark';
};

const TestResourcesRootLight: FC<
...
const TestResourcesRootDark: FC<
  Omit<ComponentProps<'svg'>, 'size'> & { size: number }
> = ({
  size,
  className,
...
export const TestResourcesRoot: FC<TestResourcesRootProps> = ({
  size = 16,
  mode = 'light',
  className,
  'aria-label': ariaLabel,
  ...
  const SvgComponent =
    mode === 'light' ? TestResourcesRootLight : TestResourcesRootDark;
