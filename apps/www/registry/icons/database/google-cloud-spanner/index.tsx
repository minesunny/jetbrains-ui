...
export type GoogleCloudSpannerProps = Omit<ComponentProps<'svg'>, 'size'> & {
  size?: 12 | 14 | 16 | 20 | 24;
  mode?: 'light' | 'dark';
};

const GoogleCloudSpannerLight: FC<
...
const GoogleCloudSpannerDark: FC<
  Omit<ComponentProps<'svg'>, 'size'> & { size: number }
> = ({
  size,
  className,
...
export const GoogleCloudSpanner: FC<GoogleCloudSpannerProps> = ({
  size = 16,
  mode = 'light',
  className,
  'aria-label': ariaLabel,
  ...
  const SvgComponent =
    mode === 'light' ? GoogleCloudSpannerLight : GoogleCloudSpannerDark;
