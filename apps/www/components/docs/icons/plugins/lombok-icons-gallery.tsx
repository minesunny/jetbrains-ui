...
const allIconNames = Object.keys(LombokIcons).filter(
  (k) => !k.endsWith('Props') && typeof (LombokIcons as any)[k] === 'function',
);
      ...
      {filteredIcons.length === 0 ? (
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-2">
          {filteredIcons.map((iconName) => {
            const IconComponent = (LombokIcons as any)[iconName];
                ...
                {isExpanded && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 z-20 w-72 p-3 bg-fd-card border rounded-xl shadow-lg">
                    ...
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <code className="flex-1 text-[9px] p-1.5 rounded bg-fd-muted font-mono truncate">{`import { ${iconName} } from '@/components/jetbrains-ui/icons/plugins/lombok';`}</code>
                      <CopyButton
                        text={`import { ${iconName} } from '@/components/jetbrains-ui/icons/plugins/lombok';`}
                      />
                    </div>
